...
const exampleEnvs = {
  PLUGIN_USERS:
    '{"lukas":{"age":16,"hobbys":["programming","r6"]},"tim":{"age":21,"hobbys":["empty inside","or is he?"]}}',
  PLUGIN_SERVERS:
    '[{"host":"one.server","lines":["oof"],"name":"Server One","port":420},{"host":"two.server","lines":["exec foo-bar"],"name":"Server Two","port":666}]',
...
const expectedReturn = {
  ...
  users: {
    lukas: {
      age: 16,
      hobbys: ["programming", "r6"],
    },
    tim: {
      age: 21,
      hobbys: ["empty inside", "or is he?"],
    },
  },
  servers: [
    {
...
describe("Parser", function () {
  describe("#parseEnvs()", function () {
    ...
    it("should use the provided prefix for the environment variables if provided", function () {
      let restore = mockedEnv(
        { EXTENSION_TEST: "a string", PLUGIN_THIS: "won\t be parsed" },
        { clear: true }
      );
      try {
        assert.deepStrictEqual(
